...
|DeleteArchivedPlaylist-ControlB| N/A | 10-DeleteArchivedPlaylist-ControlB | N/A | 26 minutes | 1 | Pass (100) | Fail | While the feature was implimented and is working with passing tests, the tool hallucinated trying to revert all assemblies to .net 8 as it thought that being on .net 10 was a mistake. This is likely due to .net 10 being released 4 days ago, however the tool was never prompted by me, nor a failing test that indicated it could by the .net version causing the issue. Additonally, the command being implemented should have been implemented asymmetrically compared to rest of the operations as it only required the Data Access Layer. However, the tool instead decided to implement through the ISpotifyService which acted as a middle man. As well as this, a failing stalled the tool for quite some time, requiring it install the third party dependancy Moq, when it was not needed or prompted to. | Fail |
